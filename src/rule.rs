...
impl<'a> Rule<'a> {
    pub(crate) fn eval_value(
        ...
        Ok(match *val {
            ...
            style::Value::Variable(ref name) if name.name == "parent_x" => {
                Value::Integer(parent_rect.x)
            }
            style::Value::Variable(ref name) if name.name == "parent_y" => {
                Value::Integer(parent_rect.y)
            }
            style::Value::Variable(ref name) if name.name == "parent_width" => {
                Value::Integer(parent_rect.width)
            }
            style::Value::Variable(ref name) if name.name == "parent_height" => {
                Value::Integer(parent_rect.height)
            }
            style::Value::Variable(ref name) => self.vars.get(&name.name).cloned().ok_or_else(
                || ErrorKind::UnknownVariable(name.name.clone(), name.position),
            )?,
        })
    }
        ...
        match expr.expr {
            ...
            style::Expr::Add(ref l, ref r) => {
                ...
                match (l, r) {
                    ...
                    _ => Err(ErrorKind::CantOp("add".into(), expr.position).into()),
                }
            }
            style::Expr::Sub(ref l, ref r) => {
                ...
                match (l, r) {
                    ...
                    (Value::Float(l), Value::Integer(r)) => Ok(Value::Float(l - r as f64)),
                    (Value::Integer(l), Value::Float(r)) => Ok(Value::Float(l as f64 - r)),
                    _ => Err(ErrorKind::CantOp("subtract".into(), expr.position).into()),
                }
            }
            style::Expr::Mul(ref l, ref r) => {
                ...
                match (l, r) {
                    ...
                    (Value::Float(l), Value::Integer(r)) => Ok(Value::Float(l * r as f64)),
                    (Value::Integer(l), Value::Float(r)) => Ok(Value::Float(l as f64 * r)),
                    _ => Err(ErrorKind::CantOp("multiply".into(), expr.position).into()),
                }
            }
            style::Expr::Div(ref l, ref r) => {
                ...
                match (l, r) {
                    ...
                    (Value::Float(l), Value::Integer(r)) => Ok(Value::Float(l / r as f64)),
                    (Value::Integer(l), Value::Float(r)) => Ok(Value::Float(l as f64 / r)),
                    _ => Err(ErrorKind::CantOp("divide".into(), expr.position).into()),
                }
            }
            style::Expr::Neg(ref l) => {
                ...
                match l {
                    ...
                    _ => Err(ErrorKind::CantOp("negate".into(), expr.position).into()),
                }
            }
            style::Expr::Call(ref name, ref args) => {
                if let Some(func) = styles.funcs.get(&name.name) {
                    ...
                    func(args).chain_err(|| ErrorKind::FunctionFailed(expr.position))
                } else {
    ...
    pub(crate) fn get_value<T, V: PropertyValue>(
        &self,
        styles: &Styles<T>,
        parent_rect: Rect,
        name: &str,
...
where
    I: Iterator<Item = &'a syntax::style::Rule> + 'a,
{
    type Item = Rule<'a>;
    fn next(&mut self) -> Option<Self::Item> {
        use syntax::style;
        'search: while let Some(rule) = self.rules.next() {
                ...
                if let Some(cur) = current.take() {
                    ...
                    for (prop, v) in &m.1 {
                        if let Some(nprop) = cur.properties.get(&prop.name) {
                            match (&v.value, nprop) {
                                (&style::Value::Variable(ref name), val) => {
                                    vars.insert(name.name.clone(), val.clone());
                                }
                                (&style::Value::Boolean(b), &Value::Boolean(nb)) if nb == b => {}
                                (&style::Value::Integer(i), &Value::Integer(ni)) if ni == i => {}
                                (&style::Value::Float(f), &Value::Float(nf)) if nf == f => {}
                                (&style::Value::String(ref s), &Value::String(ref ns))
                                    if ns == s => {}
                                _ => continue 'search,
                            }
                        } else {
